...
function draw() {
    if (x) {
        ...
        surfaceB.Draw(); //gl.clear(gl.DEPTH_BUFFER_BIT);
        gl.uniformMatrix4fv(shProgram.iModelViewProjectionMatrix, false, m4.translation(0.5 * audioPosition[0], 0.5 * audioPosition[1], 0.01 * audioPosition[2]));
        gl.bindTexture(gl.TEXTURE_2D, texture);
        if (panner) { panner.setPosition(audioPosition[0], audioPosition[1], audioPosition[2]) }
        surfaceP.Draw(); //gl.clear(gl.DEPTH_BUFFER_BIT);
        ...
        modelViewProjection = m4.multiply(cam.ProjectionMatrix, m4.multiply(cam.ModelViewMatrix, m4.multiply(matAccum1, newMatrix2)));
        gl.uniformMatrix4fv(shProgram.iModelViewProjectionMatrix, false, modelViewProjection);
        surface.Draw();
 ...
 */
let newMatrix2 = m4.identity()
let audioPosition = [0, 0, 0]
function init() {
    handleContext()
    window.addEventListener(
        "deviceorientation",
        (e) => {
            newMatrix2 = m4.multiply(
                m4.xRotation(deg2rad(e.beta)), m4.multiply(
                    m4.yRotation(deg2rad(e.gamma)),
                    m4.zRotation(deg2rad(e.alpha))))
            audioPosition = deviceorientationToAudioPosition(e.alpha, e.beta, e.gamma)
        },
        true,
    );
    gui = new GUI();
